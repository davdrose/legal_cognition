...
const CHARACTER_ORDER = sessionGet('exp2_char_order', () => shuffle3(['P', 'V', 'TRASH']));
...
/* ----------------------------------------------------------
   ...
   ---------------------------------------------------------- */
function buildFaultRatingTrial(scenario, scenarioIdx, total, pCookiesAfter) {
  const pName = scenario.p_name || 'Finn';
  const vName = scenario.v_name || 'Cleo';
...
function buildTestTrial(scenario, scenarioIdx, total) {
  ...
  const faultRatingSlide = buildFaultRatingTrial(scenario, scenarioIdx, total, pAfter);

  // Allocation screen with gate question integrated. No last-slide header
  // image — instead a small recap portrait pair (via harm_text) sits where
  // that image used to be; the interactive columns below are unchanged.
  const slideG = {
  ...
  const trials = [storySlide, slideG, faultRatingSlide, ...checkerTrials];
  return trials;
}
...
(function () {
  ...
  document.getElementById('rdp-reroll').addEventListener('click', () => {
    ...
    sessionStorage.removeItem('exp2_char_order');
    location.reload();
  });

  document.getElementById('rdp-reroll-study').addEventListener('click', () => {
    // Re-runs the same between-subjects random assignment a fresh visitor
    // would get, then jumps straight to the chosen study's own index.html
    // (dropping jumpTo — that only makes sense for the study we're on).
    const newCondition = Math.random() < 0.5 ? 'above' : 'zero';
    sessionStorage.setItem('exp2_adult_outcome_condition', newCondition);
