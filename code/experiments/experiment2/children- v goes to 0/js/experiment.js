...
const TRASH_ON_LEFT = sessionGet('exp2_children_vgo0_trash', () => Math.random() < 0.5);
...
const finnCleoScenarios = [
  {
    ...
    story_video: 'videos%20vgo0/finn%20and%20cleo%20vgo0.mp4'
  }
];
...
const secondBlockScenarios = [
  {
    ...
    story_video: 'videos%20vgo0/milo%20and%20sasha%20vgo0.mp4'
  },
  {
    id: 6,
    ...
    story_video: 'videos%20vgo0/rex%20and%20zoe%20vgo0.mp4'
  },
  {
    id: 7,
    ...
    story_video: 'videos%20vgo0/kai%20and%20ruby%20vgo0.mp4'
  },
  {
    id: 8,
    ...
    story_video: 'videos%20vgo0/sam%20and%20ella%20v%20go0.mp4'
  },
  {
    id: 9,
    ...
    story_video: 'videos%20vgo0/andy%20and%20catherine%20vgo0.mp4'
  },
  {
    id: 10,
    ...
    story_video: 'videos%20vgo0/harry%20and%20katie%20vgo0.mp4'
  },
  {
    id: 11,
    ...
    story_video: 'videos%20vgo0/nora%20and%20eric%20vgo0.mp4'
  }
];
...
const warmupLayoutLocked = {
  ...
  on_load: function() {
    ...
    audio.src = `${SHARED_BASE}If you think anyone should be punished, you can decide how they lose their cookies..m4a`;
...
/* ----------------------------------------------------------
   FAULT QUESTION TRIAL BUILDER
   Shown after the story video, before the punishment/allocation screen.
   Children pick a picture rather than move a slider: P's portrait,
   both characters together (the story's own outcome image), or V's
   portrait — recorded on the same 0/50/100 scale used by the adult
   fault-rating slider (100 = P fully at fault, 0 = V fully at fault)
...
function buildFaultQuestionTrial(scenario) {
  ...
  return {
    ...
    choices: [],
    stimulus: `
      <div style="text-align:center; padding:16px 20px 0 20px; max-width:1100px; margin:0 auto;">
        <p style="font-size:20px; font-weight:600; margin:0 0 18px 0;">Now that you saw what happened, who do you think is more at fault?</p>
        <div class="fault-pick-row">
          <button class="fault-pick-option" type="button" data-value="0">
            <img src="${CHAR_IMG_BASE}${vImg}" class="fault-pick-img" alt="${vName}">
            <div class="fault-pick-label">${vName} is more at fault</div>
          </button>
          <button class="fault-pick-option" type="button" data-value="50">
            <img src="${togetherImg}" class="fault-pick-img fault-pick-img-together" alt="${pName} and ${vName}">
            <div class="fault-pick-label">Both are equally at fault</div>
          </button>
          <button class="fault-pick-option" type="button" data-value="100">
            <img src="${CHAR_IMG_BASE}${pImg}" class="fault-pick-img" alt="${pName}">
            <div class="fault-pick-label">${pName} is more at fault</div>
          </button>
        </div>
      </div>`,
    on_load: function() {
      const startTime = performance.now();
      document.querySelectorAll('.fault-pick-option').forEach(btn => {
        btn.addEventListener('click', () => {
          jsPsych.finishTrial({
            fault_rating: parseInt(btn.dataset.value, 10),
